"""数据清洗核心逻辑 — 缺失值分析、异常值检测、手动/自动清洗。

接口契约参见 api-contract.md 第三章。
"""
...
def execute_clean(
    ...
    Args:
        df: 原始 DataFrame
        strategy: "mean" / "median" / "mode" / "drop" / "custom"
        columns: 需要处理的列名列表
        fill_value: strategy="custom" 时的填充值
...
def auto_clean(df: pd.DataFrame) -> tuple[pd.DataFrame, int, list[str]]:
    ...
    for entry in outlier_report:
        ...
        all_messages.append(
            f"{col} 列：IQR 截断 {entry['outlier_count']} 个异常值"
            f"（边界: [{lower}, {upper}]）"
        )
