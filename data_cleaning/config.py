...
def _apply_outlier_action(df: pd.DataFrame, col: str, rule: ColumnRule) -> tuple[int, str]:
    ...
    if rule.outlier_action == "clip":
        lower_val = float(q1 - rule.iqr_multiplier * (q3 - q1)) if rule.outlier_method == "iqr" else float(series.mean() - rule.zscore_threshold * series.std())
        upper_val = float(q3 + rule.iqr_multiplier * (q3 - q1)) if rule.outlier_method == "iqr" else float(series.mean() + rule.zscore_threshold * series.std())
        df[col] = df[col].clip(lower=lower_val, upper=upper_val)
        return outlier_count, f"{col} 列（规则）：IQR/Z-Score 截断 {outlier_count} 个异常值"
